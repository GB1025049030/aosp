      ...
      switch ((*b)->format) {
                ...
                treat_as_reprocess ? REPROCESS
                                   : (device_settings->second.edge_mode ==
                                      ANDROID_EDGE_MODE_HIGH_QUALITY)
                                         ? HIGH_QUALITY
                                         : REGULAR;
            ALOGE("FreemeEmulated[%s][0]: Call ProcessYUV420", __FUNCTION__);
          ...
          ProcessType process_type = treat_as_reprocess
                                         ...
                                         : (device_settings->second.edge_mode ==
                                            ANDROID_EDGE_MODE_HIGH_QUALITY)
                                               ? HIGH_QUALITY
                                               : REGULAR;
          ALOGE("FreemeEmulated[%s][1]: Call ProcessYUV420", __FUNCTION__);
          auto ret = ProcessYUV420(
              yuv_input, yuv_output, device_settings->second.gain,
              process_type, device_settings->second.zoom_ratio,
              rotate, device_chars->second);
        ...
        case PixelFormat::YCBCR_P010:
            if (!reprocess_request) {
              ...
              ALOGE("FreemeEmulated(%s)[0]: Call CaptureYUV420", __FUNCTION__);
...
status_t EmulatedSensor::ProcessYUV420(const YUV420Frame& input,
  ...
  switch (process_type) {
    case HIGH_QUALITY:
      ALOGE("FreemeEmulated(%s)[2]: Call CaptureYUV420", __FUNCTION__);
    ...
    default:
      ...
      input_planes = {
          ...
          .cbcr_step = 1};
      ALOGE("FreemeEmulated(%s)[1]: Call CaptureYUV420", __FUNCTION__);
